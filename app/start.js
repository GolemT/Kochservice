...
/*  API Request to get a specific table object in recipe
 *  
 *  @param req: int
 *  @return res: JSON
*/
...
/*  API Request to get ID, title and pic of all objects in recipe
 *  
...
/*  API Request to get the current amount of table objects in recipe
 *  
 *  @return res: int
*/
...
/*  API Request to select specific table objects from recipe
 *  
 *  @param req: JSON
 *  @return res: JSON array
*/
...
/*  API Request to create a new table object in recipe
 *  
 *  @param req: JSON
*/
app.post('/api/postRecipe', async (req, res) => {
  try{
    ...
    await connection.execute('INSERT INTO `recipe` (`title`, `pic`, `ingredients`, `preparation`) VALUES (?, ?, ?, ?)', [data.title, data.pic, data.ingredients, data.preparation])
    ...
    console.error(error)
    res.status(500).json({error: 'An error ocurred while fetching the entries'})
  }
})

/*  Defines the listening port of the backend
*/
app.listen(port, () => {
